...
static char* getTCVToIDSMappingFileName(const char* IDSRequest, int shotNumber);
...
int tcvPlugin(IDAM_PLUGIN_INTERFACE* idam_plugin_interface)
{
    int err = 0;
...
{
    const char* help = "\ntcvPlugin: this plugin maps TCV data to IDS\n\n";
    const char* desc = "tcvPlugin: help = plugin used for mapping TCV experimental data to IDS";
...
int do_read(IDAM_PLUGIN_INTERFACE* idam_plugin_interface)
{
    ...
    char* fullTCVMappingFileName =
            getTCVToIDSMappingFileName(IDSRequest, shot);
    char* mappingFileName = getenv("UDA_TCV_MAPPING_FILE");
    ...
    if (IDSRequestType == STATIC) {
        ...
        int status =
                execute_xpath_expression(fullTCVMappingFileName, xPath, data_block, indices);
        if (status != 0) {
            return status;
        }
...
char* getTCVToIDSMappingFileName(const char* IDSRequest, int shot)
{
    ...
    char* dir = getenv("UDA_TCV_MAPPING_FILE_DIRECTORY");
