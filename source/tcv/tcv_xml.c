...
int execute_xpath_expression(const char* filename, const xmlChar* xpathExpr, DATA_BLOCK* data_block, int* nodeIndices)
{
    ...
    UDA_LOG(UDA_LOG_DEBUG, "TCV: Entering in execute_xpath_expression...\n");
    ...
    UDA_LOG(UDA_LOG_DEBUG, "TCV: XML file successfully parsed.\n");
    ...
    UDA_LOG(UDA_LOG_DEBUG, "TCV: XPath context successfully created.\n");
    ...
    UDA_LOG(UDA_LOG_DEBUG, "TCV: Evaluate xpath expression for requesting the data...\n");
    ...
    UDA_LOG(UDA_LOG_DEBUG, "TCV: xpath expression successfully evaluated: %s\n", xpathExpr);
    ...
    if (size == 0) {
        UDA_LOG(UDA_LOG_DEBUG, "TCV: tcv_xml plugin : error in XPath request\n");
        fprintf(stderr, "tcv_xml plugin : error in XPath request  \n");
        xmlXPathFreeContext(xpathCtx);
        xmlFreeDoc(doc);
        return -1;
    }
    ...
    if (cur->name == NULL) {
        UDA_LOG(UDA_LOG_DEBUG, "TCV: null pointer (nodes->nodeTab[nodeindex]->name)\n");
    ...
    UDA_LOG(UDA_LOG_DEBUG, "TCV:setting UDA data_block...\n");
    ...
    if (data_type == UDA_TYPE_DOUBLE) {
        UDA_LOG(UDA_LOG_DEBUG, "TCV:conversion in double\n");
        ...
        UDA_LOG(UDA_LOG_DEBUG, "TCV:successfull conversion in double\n");
    } else if (data_type == UDA_TYPE_FLOAT) {
        UDA_LOG(UDA_LOG_DEBUG, "TCV:conversion in float\n");
        ...
        UDA_LOG(UDA_LOG_DEBUG, "TCV:successfull conversion in float\n");
        /*for (i = 0; i < dim; i++) {
   	   UDA_LOG(UDA_LOG_DEBUG, "TCV:data=%f\n", (float)atof(data[i][0]);
        }*/

    } else if (data_type == UDA_TYPE_LONG) {
        UDA_LOG(UDA_LOG_DEBUG, "TCV:conversion in long\n");
        ...
        UDA_LOG(UDA_LOG_DEBUG, "TCV:conversion in int\n");
        ...
        UDA_LOG(UDA_LOG_DEBUG, "TCV:conversion in short\n");
        ...
        UDA_LOG(UDA_LOG_DEBUG, "TCV:conversion in string\n");
        data_block->data_type = UDA_TYPE_STRING;
        data_block->data = strdup((char*)cur->children->content);
        UDA_LOG(UDA_LOG_DEBUG, "TCV:conversion in string --> %s\n", data_block->data);
    ...
    xmlXPathFreeContext(xpathCtx);
    xmlFreeDoc(doc);
    UDA_LOG(UDA_LOG_DEBUG, "TCV:returning from execute_xpath_expression\n");
