...
int source_plugin(IDAM_PLUGIN_INTERFACE* idam_plugin_interface)
{
    ...
    for (i = 0; i < request_data->nameValueList.pairCount; i++) {
        if (!strcasecmp(request_data->nameValueList.nameValue[i].name, "reset") ||
            !strcasecmp(request_data->nameValueList.nameValue[i].name, "initialise")) {
            isReset = 1;
            break;
        }
    }
    ...
    if (isReset || housekeeping || !strcasecmp(request_data->function, "reset")) {
    ...
    if (!STR_IEQUALS(request_data->function, "help") &&
        (!init || STR_IEQUALS(request_data->function, "init") || STR_IEQUALS(request_data->function, "initialise"))) {
    ...
    } else if (!strcasecmp(request_data->function, "help")) {
        err = do_help(idam_plugin_interface);
    } else if (!strcasecmp(request_data->function, "version")) {
        err = do_version(idam_plugin_interface);
    } else if (!strcasecmp(request_data->function, "builddate")) {
        err = do_builddate(idam_plugin_interface);
    } else if (!strcasecmp(request_data->function, "defaultmethod")) {
        err = do_defaultmethod(idam_plugin_interface);
    } else if (!strcasecmp(request_data->function, "maxinterfaceversion")) {
...
static int do_get(IDAM_PLUGIN_INTERFACE* idam_plugin_interface, int* timeCountCache, char** timeCache, char** dataCache)
{
    ...
    bool isData = findValue(&request_data->nameValueList, "data");

    // Only return the time - ignore the data
    bool isTime = findValue(&request_data->nameValueList, "time");
    ...
    bool isNoCacheTime = findValue(&request_data->nameValueList, "nocacheTime");

    //isNoCacheData = findValue(&request_data->nameValueList, "nocacheData");

    const char* signal = NULL;
    FIND_REQUIRED_STRING_VALUE(request_data->nameValueList, signal);

    const char* source = NULL;
    bool isSource = FIND_STRING_VALUE(request_data->nameValueList, source);

    const char* format = NULL;
    bool isFormat = findStringValue(&request_data->nameValueList, &format, "format|pattern");

    const char* owner = NULL;
    bool isOwner = FIND_STRING_VALUE(request_data->nameValueList, owner);

    int shotNumber = 0;
    bool isShotNumber = findIntValue(&request_data->nameValueList, &shotNumber, "shotNumber|shot|pulse|exp_number");

    int runNumber = -1;
    bool isRunNumber = findIntValue(&request_data->nameValueList, &runNumber, "runNumber|run|pass|sequence");

#ifdef FIND_DOUBLE_VALUE
    double dataScaling = 1.0;
    bool isDataScaling = FIND_DOUBLE_VALUE(request_data->nameValueList, dataScaling);

    double timeScaling = 1.0;
    bool isTimeScaling = FIND_DOUBLE_VALUE(request_data->nameValueList, timeScaling);
#else
    float dataScaling = 1.0;
    bool isDataScaling = FIND_FLOAT_VALUE(request_data->nameValueList, dataScaling);

    float timeScaling = 1.0;
    bool isTimeScaling = FIND_FLOAT_VALUE(request_data->nameValueList, timeScaling);
#endif

    const char* host = NULL;
    bool isHost = FIND_STRING_VALUE(request_data->nameValueList, host);

    int port = 0;
    bool isPort = FIND_INT_VALUE(request_data->nameValueList, port);
    ...
    strcpy(next_request_data.signal, signal);            // Prepare the API arguments
    if (!isShotNumber) {
        shotNumber = request_data->exp_number;
    }
    ...
    if (isFormat && !strcasecmp(format, "ppf")) {
        ...
        if (env == NULL) {
            sprintf(next_request_data.source, "PPF%s/%s/%d", request_data->api_delim, source, shotNumber);
        } else {
            sprintf(next_request_data.source, "%s%sPPF%s/%s/%d", env, request_data->api_delim,
                    request_data->api_delim, source, shotNumber);
        }

        if (isRunNumber) {
            sprintf(next_request_data.source, "%s/%d", next_request_data.source, runNumber);
        } else {
            sprintf(next_request_data.source, "%s/0", next_request_data.source);
        }

        if (isOwner) {
            sprintf(next_request_data.source, "%s/%s", next_request_data.source, owner);
        }
        ...
        if (env == NULL) {
            sprintf(next_request_data.source, "JPF%s%d", request_data->api_delim, shotNumber);
        } else {
            sprintf(next_request_data.source, "%s%sJPF%s%d", env, request_data->api_delim, request_data->api_delim,
                    shotNumber);
        }

        UDA_LOG(UDA_LOG_DEBUG, "signal #JPF = %s\n", next_request_data.signal);
        UDA_LOG(UDA_LOG_DEBUG, "source #JPF = %s\n", next_request_data.source);
        ...
        if (!isShotNumber && !isRunNumber) {
            ...
            if (env == NULL) {
                strcpy(next_request_data.source, request_data->source);
            } else {
                sprintf(next_request_data.source, "%s%s%s", env, request_data->api_delim, request_data->source);
            }
        } else {
            if (env == NULL) {
                sprintf(next_request_data.source, "%d", shotNumber);
            } else {
                sprintf(next_request_data.source, "%s%s%d", env, request_data->api_delim, shotNumber);
            }
            if (isRunNumber) sprintf(next_request_data.source, "%s/%d", next_request_data.source, runNumber);
        }
        ...
        if (!isShotNumber && !isRunNumber) {
            ...
            if (isSource) {
                strcpy(next_request_data.source, source);
            }
        } else if(request_data->source[0] != '\0') {
	       strcpy(next_request_data.source, request_data->source);
        } else {
            sprintf(next_request_data.source, "%d", shotNumber);
            if (isRunNumber) sprintf(next_request_data.source, "%s/%d", next_request_data.source, runNumber);
        ...
        if (isSource) {
            ...
            if (env == NULL) {
                sprintf(next_request_data.source, "MDSPLUS%s%s/%s/%d", request_data->api_delim, host, source,
                        shotNumber);
            } else {
                sprintf(next_request_data.source, "%s%s%s/%s/%d", env, request_data->api_delim, host, source,
                        shotNumber);
            }
        } else {
            if (env == NULL) {
                sprintf(next_request_data.source, "MDSPLUS%s%s", request_data->api_delim, host);
            } else {
                sprintf(next_request_data.source, "%s%s%s", env, request_data->api_delim, host);
            }
            char* p = NULL;
        ...
        if (isHost) {
            if (isPort) {
                sprintf(new_signal, "%s::get(host=%s, port=%d, signal=\"%s\", source=\"%s\")", env, host, port,
                        next_request_data.signal, next_request_data.source);
            } else {
            ...
            if (isPort) {
                ...
                sprintf(new_signal, "%s::get(host=%s, port=%d, signal=\"%s\", source=\"%s\")", env, server_host,
                        server_port, next_request_data.signal, next_request_data.source);
            }
        }
    } else {
        if (isHost) {
    ...
    strcpy(next_request_data.signal, new_signal);

    UDA_LOG(UDA_LOG_DEBUG, "signal #1 = %s\n", next_request_data.signal);
    UDA_LOG(UDA_LOG_DEBUG, "source #1 = %s\n", next_request_data.source);

    make_request_data(&next_request_data, *pluginList, idam_plugin_interface->environment);

    char api_signal[STRING_LENGTH];
    char api_source[STRING_LENGTH];

    // These are what are used to access data - retain as cache keys
    strcpy(api_signal, next_request_data.signal);
    strcpy(api_source, next_request_data.source);

    // Call the UDA client via the UDA plugin (ignore the request identified)

    if (env != NULL) {
        next_request_data.request = findPluginRequestByFormat(env, pluginList);
    } else {
        next_request_data.request = findPluginRequestByFormat("UDA", pluginList);
    }
    ...
    if (!skipPlugin) {
        int id = findPluginIdByRequest(next_request_data.request, pluginList);
